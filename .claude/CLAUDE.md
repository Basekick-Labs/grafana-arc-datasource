...
9. **Template-variable interpolation must match Grafana's own SQL datasources exactly.** The spec is `packages/grafana-sql/src/datasource/SqlDatasource.ts` `interpolateVariable`: a string value is wrapped in quotes ONLY when `variable.multi || variable.includeAll`; otherwise it is escaped (`replace(/'/g, "''")`) with no surrounding quotes. Dashboards write the quotes themselves (`host = '$server'`, `host ~ '^$server$'`, `INTERVAL '$__interval'`) and rely on multi variables being quoted (`AND cpu = $cpu`). Any deviation breaks one idiom or the other. In 1.3.2 a security finding (R2-HI5) claimed "the Postgres datasource quotes unconditionally" — that is false, and acting on it cost nine broken releases. **Never change interpolation semantics without diffing against the upstream file first**, and never invent a heuristic (literal-walking, edge-matching) in place of the rule.

10. **A security finding that changes interpolation or macro-expansion semantics requires an upstream parity check before implementation.** Hardening that rejects input the previous version accepted ships default-off, or waits for a major version. See the 1.3.2 audit in `docs/progress/` for the full list of what "harden everything" broke.
